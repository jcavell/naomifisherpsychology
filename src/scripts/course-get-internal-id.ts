// Auto-generated file
export interface CourseData {
  title: string;
  offerId: string;
  internalId: string;
}

export const courses: CourseData[] = [
  {
    title: "All 10 in The Art Of Low Demand Parenting series",
    offerId: "uyN66L7f",
    internalId: "2149240640",
  },
  {
    title: "First 5 in The Art Of Low Demand Parenting series",
    offerId: "NnpWyFh4",
    internalId: "2149583101",
  },
  {
    title: "6-10 in The Art Of Low Demand Parenting series",
    offerId: "9dcEAsUU",
    internalId: "2149246615",
  },
  {
    title: "The Art of Low Demand Parenting: Activate Your Parenting",
    offerId: "qJbUaMox",
    internalId: "2149348108",
  },
  {
    title: "The Art Of Low Demand Parenting: Communication",
    offerId: "PiddLUVy",
    internalId: "2149409572",
  },
  {
    title: "The Art of Low Demand Parenting: Behaviour",
    offerId: "fEB525it",
    internalId: "2149446569",
  },
  {
    title: "The Art of Low Demand Parenting: Emotions",
    offerId: "GCa7Yn7t",
    internalId: "2149583080",
  },
  {
    title: "The Art of Low Demand Parenting: The Real World",
    offerId: "9mEzpCdy",
    internalId: "2148820488",
  },
  {
    title: "The Art of Low Demand Parenting: Other People",
    offerId: "7FLvyocu",
    internalId: "2148933332",
  },
  {
    title: "The Art of Low Demand Parenting: Screens",
    offerId: "UgeLqYii",
    internalId: "2148962255",
  },
  {
    title: "The Art of Low Demand Parenting: School",
    offerId: "iSyYWDbg",
    internalId: "2149049975",
  },
  {
    title: "The Art of Low Demand Parenting: What About the Future?",
    offerId: "ME3A2Bqr",
    internalId: "2149118868",
  },
  {
    title: "The Art of Low Demand Parenting: What About Me?",
    offerId: "4hZ9xFzR",
    internalId: "2149186072",
  },
  {
    title: "Demand Avoidance 101",
    offerId: "LdxQoHr6",
    internalId: "2149124019",
  },
  {
    title: "Understanding and Helping your Demand Avoidant Adolescent",
    offerId: "ktF9zj5g",
    internalId: "2149599386",
  },
  {
    title: "Helping Your Autistic Child With Transitions",
    offerId: "Zsdtfjk7",
    internalId: "2149911058",
  },
  {
    title: "Helping Your Autistic Child With OCD",
    offerId: "PmC2E3cZ",
    internalId: "2149910889",
  },
  {
    title: "Helping Your Autistic Child With Anxiety",
    offerId: "PLws5GJk",
    internalId: "2149023580",
  },
  {
    title: "Helping Your Autistic Teen with Anxiety",
    offerId: "2niYSiJb",
    internalId: "2149066471",
  },
  {
    title: "Neurodiversity 101",
    offerId: "TeyY9BM2",
    internalId: "2150071471",
  },
  {
    title:
      "Helping Your Autistic Child Develop a Healthy Relationship with Screens ",
    offerId: "5Q9LLqoJ",
    internalId: "2149345797",
  },
  {
    title: " Helping Your Autistic Child with Trauma",
    offerId: "WgZFAJMu",
    internalId: "2149345795",
  },
  {
    title: "Helping Your Child with Anxiety (under 8s)",
    offerId: "HFc3ZvHE",
    internalId: "2148996932",
  },
  {
    title: "Helping Your Child with Anxiety (ages 6-13)",
    offerId: "fD2DLM2F",
    internalId: "2148992687",
  },
  {
    title: "Helping Your Child with Severe Anxiety",
    offerId: "RN3w75TE",
    internalId: "2148992732",
  },
  {
    title: "Helping Your Child Recover From Trauma And Loss",
    offerId: "8mo2FoE7",
    internalId: "2148641115",
  },
  {
    title: "Helping Your Child with Learning Disabilities with Anxiety",
    offerId: "DCUMFc2w",
    internalId: "2149712879",
  },
  {
    title: "EOTAS: What to Expect",
    offerId: "vV8BeUKS",
    internalId: "2149909144",
  },
  {
    title: "My Child Is Not Fine At School",
    offerId: "qRTfapFL",
    internalId: "2148443927",
  },
  {
    title: "Burnt Out by School?",
    offerId: "CXNPs8L3",
    internalId: "2148412562",
  },
  {
    title:
      "After School: Helping Your Autistic Child Move On from School Trauma",
    offerId: "xvn8Y7TJ",
    internalId: "2148368194",
  },
  {
    title: "Helping Autistic Children With School",
    offerId: "C746RNzF",
    internalId: "2149111759",
  },
  {
    title: "Helping Your Autistic Child with Starting School",
    offerId: "5kF79gzP",
    internalId: "2149043294",
  },
  {
    title: "Navigating the SEN Tribunal process",
    offerId: "jLbWvRoa",
    internalId: "2149793175",
  },
  {
    title: "Do we need an EHCP? If we do, what do we do now?",
    offerId: "7boherxL",
    internalId: "2150009684",
  },
  {
    title: "Taking Control of Your EHCP Annual Review",
    offerId: "NzaULaoT",
    internalId: "2149524840",
  },
  {
    title: "We don't have an EHCP - what can we expect from school?",
    offerId: "f5rVFA5f",
    internalId: "2150009686",
  },
  {
    title: "Finding Your Way Through the EHCP Jungle",
    offerId: "m2QJJLso",
    internalId: "2149202300",
  },
  {
    title: "Thriving After School Breakdown: An Illustrated Talk",
    offerId: "PdVHfz9J",
    internalId: "2148374682",
  },
  {
    title: "Burnout - A Guide for Teenagers",
    offerId: "sVWpRtm2",
    internalId: "2149330215",
  },
  {
    title: "What is Demand Avoidance? A Guide for Teenagers",
    offerId: "36wrrznS",
    internalId: "2149522995",
  },
  {
    title:
      "Working with Demand Avoidant Children: The Art and Science of Low Demand Therapy",
    offerId: "dZKvi4pT",
    internalId: "2149345800",
  },
  {
    title: "EMDR Refresher Training Bundle - All 6 mini courses",
    offerId: "hEUj2ZLB",
    internalId: "2149379574",
  },
  {
    title: "EMDR Refresher Training - Assessment Phase",
    offerId: "ANz4X47o",
    internalId: "2149251496",
  },
  {
    title: "EMDR Refresher Training - Blocked Processing",
    offerId: "CE7NArBG",
    internalId: "2149263825",
  },
  {
    title: "EMDR Refresher Training - Cognitive Interweaves",
    offerId: "bnHT9yzQ",
    internalId: "2149263828",
  },
  {
    title: "EMDR Refresher Training - Preparation Phase",
    offerId: "RrQhW3pq",
    internalId: "2149379547",
  },
  {
    title: "EMDR Refresher Training - Desensitisation Phase",
    offerId: "znLNPWfF",
    internalId: "2149379558",
  },
  {
    title: "EMDR Refresher Training - Phases 5-8",
    offerId: "Zg2oKtDF",
    internalId: "2149379564",
  },
  {
    title: "My Child is Not Fine at School. Now What?",
    offerId: "CPxUdSiV",
    internalId: "2150254624",
  },
  {
    title: "Helping your Autistic Child with Anger",
    offerId: "cspb95bK",
    internalId: "2150219275",
  },
  {
    title: "Helping your Neurodivergent Teen Recover from School",
    offerId: "EzLuffn4",
    internalId: "2150195750",
  },
  {
    title: "Autism 101",
    offerId: "LVVDaeSp",
    internalId: "2150071498",
  },
  {
    title:
      "Understanding Dyslexia and Reading Difficulties: An Introduction for Parents",
    offerId: "VFEv5rR3",
    internalId: "2150119077",
  },
  {
    title: "Helping Your Demand Avoidant Child With Change",
    offerId: "L2VrBCuX",
    internalId: "2150129038",
  },
  {
    title: "We've Got Their Diagnosis. Now What?",
    offerId: "LYowpEmo",
    internalId: "2150187878",
  },
  {
    title:
      "An Introduction to Difficulties with Maths, including Dyscalculia and Maths Anxiety",
    offerId: "Rx2spkER",
    internalId: "2150190447",
  },
  {
    title: "They're Not Going to School. Now What?",
    offerId: "YoPepFkv",
    internalId: "2150129091",
  },
  {
    title: "Burnt out by Parenting?",
    offerId: "vDStPgYr",
    internalId: "2150188031",
  },
  {
    title: "Low Pressure Parenting for Teens",
    offerId: "LgLTXCm7",
    internalId: "2150187934",
  },
  {
    title: "Helping your Child with Burnout",
    offerId: "52TX3LQu",
    internalId: "2150160530",
  },
  {
    title: "How Can You Learn if You Don't Go to School? A Guide for Teenagers",
    offerId: "XgLDdAFa",
    internalId: "2150049763",
  },
  {
    title: "Is It All My Fault?",
    offerId: "4TZvzrYn",
    internalId: "2149873959",
  },
  {
    title: "Helping Your Teen With Anxiety",
    offerId: "3qPhcXAK",
    internalId: "2149997822",
  },
];

export function getInternalIdFromOfferId(offerId: string): string | undefined {
  return courses.find((course) => course.offerId === offerId)?.internalId;
}
